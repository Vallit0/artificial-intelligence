...
const LessonPath = ({
  ...
  return <div className="flex flex-col items-center py-4 sm:py-8 space-y-1 sm:space-y-2 pb-24 lg:pb-8">
      ...
      return <div key={scenario.id} className="flex flex-col items-center">
            ...
            {isNewSection(index) && <div className="w-full bg-secondary rounded-xl sm:rounded-2xl p-3 sm:p-4 mb-4 sm:mb-6">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                    <Button variant="ghost" size="icon" className="text-secondary-foreground hover:bg-secondary-foreground/10 h-7 w-7 sm:h-8 sm:w-8 shrink-0" onClick={() => navigate(-1)}>
                  ...
                  <Button variant="outline" size="sm" className="bg-secondary-foreground text-secondary font-bold hover:bg-secondary-foreground/90 border-0 text-xs sm:text-sm px-2 sm:px-3 h-8 shrink-0" onClick={handleGuidebookClick}>
            ...
            {index > 0 && !isNewSection(index) && <svg width="40" height="28" viewBox="0 0 60 40" className="text-muted-foreground/40 sm:w-[60px] sm:h-[40px]">
            ...
            <div className={cn("transition-transform duration-300 flex flex-col items-center", getOffsetClass(position))}>
              ...
              {isCurrent && <div className="relative mb-1 sm:mb-2 animate-bounce">
              ...
              <button onClick={() => !isLocked && handleNodeClick(scenario)} disabled={isLocked} className={cn("relative transition-all duration-300", !isLocked && "hover:scale-105 active:scale-95")}>
                {/* Outer dark ring */}
                <div className={cn("w-16 h-16 sm:w-24 sm:h-24 rounded-full flex items-center justify-center", isLocked ? "bg-muted/50" : "bg-muted shadow-lg")}>
                  {/* Inner colored circle */}
                  <div className={cn("w-12 h-12 sm:w-[72px] sm:h-[72px] rounded-full flex items-center justify-center shadow-inner", isLocked ? "bg-muted" : isCompleted ? "bg-secondary" : "bg-secondary")}>
                    {/* Icon */}
                    {isLocked ? <Lock className="w-5 h-5 sm:w-8 sm:h-8 text-muted-foreground" /> : isCompleted ? <Crown className="w-5 h-5 sm:w-8 sm:h-8 text-secondary-foreground" /> : <Star className="w-5 h-5 sm:w-8 sm:h-8 text-secondary-foreground" />}
                  </div>
                </div>

                {/* Progress ring overlay */}
                {!isLocked && !isCompleted && progress > 0 && <svg className="absolute inset-0 w-full h-full -rotate-90" viewBox="0 0 64 64 sm:0 0 96 96">
                    <circle cx="32" cy="32" r="28" fill="none" stroke="currentColor" strokeWidth="3" className="text-gold sm:hidden" strokeLinecap="round" strokeDasharray={2 * Math.PI * 28} strokeDashoffset={2 * Math.PI * 28 * (1 - progress / 100)} />
                    <circle cx="48" cy="48" r="44" fill="none" stroke="currentColor" strokeWidth="4" className="text-gold hidden sm:block" strokeLinecap="round" strokeDasharray={2 * Math.PI * 44} strokeDashoffset={2 * Math.PI * 44 * (1 - progress / 100)} />
                  </svg>}

                {/* Completion badge */}
                {isCompleted && <div className="absolute -bottom-0.5 -right-0.5 sm:-bottom-1 sm:-right-1 w-6 h-6 sm:w-8 sm:h-8 bg-gold rounded-full flex items-center justify-center shadow-md border-2 border-sidebar-background">
                    <Check className="w-3.5 h-3.5 sm:w-5 sm:h-5 text-primary" />
                  </div>}
              </button>

              {/* Scenario name */}
              <p className={cn("mt-2 sm:mt-3 text-xs sm:text-sm font-medium text-center max-w-[90px] sm:max-w-[120px] leading-tight", isLocked ? "text-muted-foreground" : "text-foreground")}>
                {scenario.name}
              </p>
            </div>
          </div>;
