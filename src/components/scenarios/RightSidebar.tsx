...
const RightSidebar = ({
  streak = 0,
  crowns = 0,
  moodleUrl = "https://moodle.senoriales.org/my/",
  practiceDays = []
}: RightSidebarProps) => {
  ...
  return <aside className="hidden xl:flex flex-col w-80 min-h-screen p-5 space-y-5 fixed right-0 top-0 bottom-0 overflow-y-auto bg-background">
      {/* Top Stats Bar */}
      <div className="gap-4 py-3 px-2 bg-card rounded-2xl border border-border flex items-center justify-center">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between gap-1">
                ...
                return <div key={index} className="flex flex-col items-center gap-1">
                      ...
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${practiced ? "bg-orange-500 text-white" : "bg-muted text-muted-foreground"} ${isToday ? "ring-2 ring-secondary ring-offset-2 ring-offset-background" : ""}`}>
                        {practiced ? <Check className="w-4 h-4" /> : <span className="text-xs">{format(day, "d")}</span>}
                      </div>
                    </div>;
              })}
              </div>
      ...
      <Card className="bg-card border-border shadow-sm hover:shadow-md transition-shadow">
        ...
        <CardContent className="space-y-4">
          ...
          return <div key={mission.id} className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isComplete ? "bg-secondary/20" : "bg-muted"}`}>
      ...
      <Card className="bg-gradient-to-br from-primary via-primary to-primary/90 border-0 text-primary-foreground overflow-hidden shadow-lg">
        <CardContent className="p-5 relative">
          ...
          <div className="relative z-10">
            ...
            <Button variant="secondary" className="w-full font-bold shadow-md hover:shadow-lg transition-shadow" onClick={() => window.open(moodleUrl, '_blank')}>
