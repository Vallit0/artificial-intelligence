...
node tests/load/node/stress.mjs

# Customización
node tests/load/node/stress.mjs --users=100 --bulk=25 --bulks=2 --read-secs=30 --concurrency=10

# Sin cleanup automático (deja los users para inspección manual)
node tests/load/node/stress.mjs --no-cleanup

# Dry run (sólo imprime config, no manda requests)
node tests/load/node/stress.mjs --dry-run
```
...
node tests/load/node/cleanup.mjs --dry-run   # ver qué borraría
node tests/load/node/cleanup.mjs              # borrar
```

```bash
# 2. Por SQL directo en la VM (fallback)
docker exec -i artificial-intelligence-db-1 psql -U senoriales -d senoriales \
  < tests/load/node/cleanup.sql
```
...
Stdout muestra tabla de percentiles por endpoint al final. Reporte JSON completo en `tests/load/node/reports/stress-<runId>.json`.
