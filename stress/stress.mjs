...
//   node stress/stress.mjs [--users=200] [--bulk=50] [--read-secs=60] [--concurrency=20] [--no-cleanup]
//
// Salida: stdout + stress/reports/stress-<runId>.json
...
if (cleanupReport.failed > 0) {
  ...
  console.log(`   node stress/cleanup.mjs (limpia por patrón)`);
  console.log(`   o ejecutá stress/cleanup.sql en la DB`);
  process.exit(1);
}
